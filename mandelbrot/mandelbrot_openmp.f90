...
!  ADAPTED FOR Talapas Tutorial: 18 Feb 2021 
!       by AI KUBO HUTCHISON
!
!  Author:
!
!    John Burkardt
...
!
! from
! https://people.sc.fsu.edu/~jburkardt/f_src/mandelbrot_openmp/mandelbrot_openmp.html
    ...
    do j = 1, n
      ...
      if ( mod ( count(i,j), 2 ) == 1 ) then
        ...
        c = int ( 255.0D+00 * sqrt ( sqrt ( sqrt ( &
          ( real ( count(i,j), kind = 8 ) &
          / real ( count_max, kind = 8 ) ) ) ) ) )
        r(i,j) = 3 * c / 5
        g(i,j) = 3 * c / 5
        b(i,j) = c
      end if
